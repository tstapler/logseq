...
import com.logseq.kmp.parsing.lexer.Lexer
import com.logseq.kmp.parsing.lexer.TokenType
...
class ParsingPropertyTest {
    ...
    @Test
    fun logseqParserShouldNotCrashOnArbitraryMarkdown() {
        repeat(1000) { i ->
            val input = generateMarkdown((i % 500))
            try {
                LogseqParser().parse(input)
            } catch (e: Exception) {
                fail("Failed for input length ${input.length}")
            }
        }
    ...
    @Test
    fun logseqParserShouldHandleEdgeCaseMarkdown() {
