package com.logseq.kmp.editor.viewmodel

import androidx.compose.runtime.*
import com.logseq.kmp.editor.state.*
import com.logseq.kmp.editor.input.ImeCompositionManager
import com.logseq.kmp.editor.input.KeyboardEventHandler
...
import com.logseq.kmp.editor.text.TextState
...
class EditorViewModel(
     ...
     * Update editor theme.
     */
    fun updateTheme(themeMode: LogseqThemeMode) {
        val currentConfig = _editorState.value.config
        val theme = when (themeMode) {
            LogseqThemeMode.LIGHT -> EditorTheme.Light
            LogseqThemeMode.DARK -> EditorTheme.Dark
            LogseqThemeMode.SYSTEM -> EditorTheme.Default
        }
                        ...
                        currentState.withImeComposition(
                            ...
                            compositionRange = compositionState.let { state ->
                                if (state.isComposing) {
                                    com.logseq.kmp.editor.text.TextRange(
                                        state.position,
    ...
    fun handleKeyboardEvent(
        key: String,
        modifiers: Set<com.logseq.kmp.editor.input.KeyModifier> = emptySet()
    ): Boolean {
