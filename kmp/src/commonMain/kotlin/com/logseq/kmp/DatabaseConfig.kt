...
object DatabaseConfig {
    ...
    private fun getCurrentVersion(driver: SqlDriver): Long {
        return try {
            val result = driver.executeQuery(null, "PRAGMA user_version;", 0)
            if (result.next()) result.getLong(0) else 0L
        } catch (e: Exception) {
            0L // If table doesn't exist, assume version 0
        }
    }
    ...
    private fun recreateTables(driver: SqlDriver, database: LogseqDatabase) {
        ...
        driver.execute(null, "DROP TABLE IF EXISTS block_references;", 0)
        driver.execute(null, "DROP TABLE IF EXISTS properties;", 0)
        driver.execute(null, "DROP TABLE IF EXISTS blocks;", 0)
        driver.execute(null, "DROP TABLE IF EXISTS pages;", 0)
    ...
    fun getDatabaseStats(database: LogseqDatabase): DatabaseStats {
        val driver = database.pageQueries.driver

        val pageCount = driver.executeQuery(null, "SELECT COUNT(*) FROM pages;", 0)
            .let { if (it.next()) it.getLong(0) else 0L }

        val blockCount = driver.executeQuery(null, "SELECT COUNT(*) FROM blocks;", 0)
            .let { if (it.next()) it.getLong(0) else 0L }

        val propertyCount = driver.executeQuery(null, "SELECT COUNT(*) FROM properties;", 0)
            .let { if (it.next()) it.getLong(0) else 0L }

        val referenceCount = driver.executeQuery(null, "SELECT COUNT(*) FROM block_references;", 0)
            .let { if (it.next()) it.getLong(0) else 0L }
     ...
     * Clean shutdown
     */
    fun shutdown(database: LogseqDatabase) {
        try {
            database.pageQueries.driver.close()
        } catch (e: Exception) {
...
<parameter name="filePath">kmp/src/commonMain/kotlin/com/logseq/kmp/DatabaseConfig.kt